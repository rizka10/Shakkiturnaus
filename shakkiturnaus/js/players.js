...
function calcScores() {
  ...
  appState.rounds.forEach(rnd => {
    rnd.pairs.forEach(pair => {
      if (pair.bId === null) {
        // BYE
        if (pair.res === '1') sc[pair.wId] = (sc[pair.wId] || 0) + appState.cfg.bye;
      } else {
        if (pair.res === '1') {
...
function calcBuchholz(sc) {
  const bh = {};
  appState.players.forEach(p => { bh[p.id] = 0; });

  appState.rounds.forEach(rnd => {
    rnd.pairs.forEach(pair => {
      if (pair.bId === null) return; // bye ignored in BH (or add bye score)
